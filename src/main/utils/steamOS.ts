...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} santi.discord Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} santi.discord's recommended Steam Deck controller settings?
